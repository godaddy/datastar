...
UpdateStatement.prototype.build = function (options) {
  ...
  for (var i = 0; i < this.buffer.length; i++) {
    ...
    if (partialStatement.cql.length) {
      ...
      //
      this.statements.push(this.finalizeStatement(table, partialStatement));
    }
  }
...
//
UpdateStatement.prototype.finalizeStatement = function (table, partial) {
  var statement = new Statement(this.schema);
  if (!partial.delete) {
    statement.cql += 'UPDATE ' + table + ' SET ' +
