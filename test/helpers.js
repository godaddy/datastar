...
exports.load = function (env, callback) {
  ...
  function createKeyspace(data) {
    ...
    client.execute(`CREATE KEYSPACE IF NOT EXISTS ${data.cassandra.keyspace}
      WITH replication = {'class' : 'SimpleStrategy', 'replication_factor' : 1};`, function (err) {
  ...
  // that file.
  //
  var configFile = process.env.DATASTAR_CONFIG
